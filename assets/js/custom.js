...
// TODO! Handle Removing Preloader
setTimeout(function () {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Global Variables
  const isPWA = true // Enables or disables the service worker and PWA
  // eslint-disable-next-line no-unused-vars
  const isAJAX = false // AJAX transitions. Requires local server or server
  const pwaName = 'Sticky' // Local Storage Names for PWA
  const pwaRemind = 1 // Days to re-remind to add to home
  const pwaNoCache = false // Requires server and HTTPS/SSL. Will clear cache with each visit
  ...
  function init_template () {
    ...
    // eslint-disable-next-line no-unused-vars
    let i, e, el // https://www.w3schools.com/js/js_performance.asp

    // Attaching Menu Hider
    const menuHider = document.getElementsByClassName('menu-hider')
    if (!menuHider.length) { const hider = document.createElement('div'); hider.setAttribute('class', 'menu-hider'); document.body.insertAdjacentElement('beforebegin', hider) }
    ...
    if (inputField.length) {
      ...
      const phoneValidator = /^[(]{0,1}[0-9]{3}[)]{0,1}[-\s\.]{0,1}[0-9]{3}[-\s\.]{0,1}[0-9]{4}$/
      ...
      const regularField = document.querySelectorAll('.input-style input:not([type="date"])')
      ...
      const regularTextarea = document.querySelectorAll('.input-style textarea')
      ...
      const selectField = document.querySelectorAll('.input-style select')
      ...
      const dateField = document.querySelectorAll('.input-style input[type="date"]')
      ...
      const validateField = document.querySelectorAll('.validate-field input, .validator-field textarea')
    ...
    // Map Page
    const fullMap = document.querySelectorAll('.map-full')
    if (fullMap.length) {
      const mapActivator = document.querySelectorAll('.show-map')
      const mapDisabler = document.querySelectorAll('.hide-map')
    ...
    // To Do List
    const toDoList = document.querySelectorAll('.todo-list a')
    ...
    if (menus.length) {
      ...
      const wrappers = document.querySelectorAll('.header, #footer-bar, .page-content')
      ...
      menuOpen.forEach(el => el.addEventListener('click', (e) => {
        ...
        // Open Clicked Menu
        const menuData = el.getAttribute('data-menu')
      ...
      menuClose.forEach(el => el.addEventListener('click', (e) => {
        ...
        iframes.forEach((el) => { const hrefer = el.getAttribute('src'); el.setAttribute('newSrc', hrefer); el.setAttribute('src', ''); const newSrc = el.getAttribute('newSrc'); el.setAttribute('src', newSrc) })
      }))
    }
    ...
    function card_extender () {
      let headerHeight, footerHeight, headerOnPage
      ...
      for (let i = 0; i < cards.length; i++) {
        if (cards[i].getAttribute('data-card-height') === 'cover') {
          if (window.matchMedia('(display-mode: fullscreen)').matches) { const windowHeight = window.outerHeight }
          if (!window.matchMedia('(display-mode: fullscreen)').matches) { const windowHeight = window.innerHeight }
          // Fix for iOS 15 pages with data-height="cover"
          const coverHeight = windowHeight + 'px'
    ...
    if (rememberHighlight) {
      ...
      const bodyHighlight = document.body.getAttribute('data-highlight')
      const defaultHighlight = bodyHighlight.split('highlight-')
    ...
    const gradientData = document.querySelectorAll('[data-change-background]')
    gradientData.forEach(el => el.addEventListener('click', (e) => {
      const gradient = el.getAttribute('data-change-background')
    ...
    const pageBackground = localStorage.getItem(pwaName + '-Gradient')
    ...
    function removeTransitions () { const falseTransitions = document.querySelectorAll('.btn, .header, #footer-bar, .menu-box, .menu-active'); for (let i = 0; i < falseTransitions.length; i++) { falseTransitions[i].style.transition = 'all 0s ease' } }
    function addTransitions () { const trueTransitions = document.querySelectorAll('.btn, .header, #footer-bar, .menu-box, .menu-active'); for (let i = 0; i < trueTransitions.length; i++) { trueTransitions[i].style.transition = '' } }
    ...
    darkModeSwitch.forEach(el => el.addEventListener('click', (e) => {
      if (document.body.className === 'theme-light') { removeTransitions(); activateDarkMode() } else if (document.body.className === 'theme-dark') { removeTransitions(); activateLightMode() }
    ...
    if (locationBut.length) {
      const locationSupport = document.getElementsByClassName('location-support')[0]
      ...
      function geoLocate () {
        ...
        function success (position) {
          ...
          const mapL1 = 'https://www.google.com/maps/embed/v1/view?key=AIzaSyAM3nxDVrkjyKwdIZp8QOplmBKLRVI5S_Y&center='
          const mapL2 = latitude + ','
          const mapL3 = longitude
          const mapL4 = '&zoom=16&maptype=satellite'
          const mapL5 = ''
          const mapLinkEmbed = mapL1 + mapL2 + mapL3 + mapL4
          const mapLinkAddress = mapL1 + mapL2 + mapL3 + mapL5
          const mapLinkNewAPI = 'https://www.google.com/maps/@' + latitude + ',' + longitude + ',15z'
        ...
        if (!navigator.geolocation) { locationCoordinates.textContent = 'Geolocation is not supported by your browser' } else { locationCoordinates.textContent = 'Locating'; navigator.geolocation.getCurrentPosition(success, error) }
      }
      const getLocation = document.getElementsByClassName('get-location')[0]
    ...
    const checkVisited = document.querySelectorAll('.check-visited')
    if (checkVisited.length) {
      function check_visited_links () {
        const visited_links = JSON.parse(localStorage.getItem(pwaName + '_Visited_Links')) || []
        const links = document.querySelectorAll('.check-visited a')
        for (let i = 0; i < links.length; i++) {
          const that = links[i]
          that.addEventListener('click', function (e) {
            const clicked_url = this.href
            if (!visited_links.includes(clicked_url)) {
              visited_links.push(clicked_url)
              localStorage.setItem(pwaName + '_Visited_Links', JSON.stringify(visited_links))
            }
          })
          if (visited_links.includes(that.href)) {
    ...
    // Scroll Ads
    const scrollItems = document.querySelectorAll('.scroll-ad, .header-auto-show')
    if (scrollItems.length) {
      ...
      window.addEventListener('scroll', function () {
        if (document.querySelectorAll('.scroll-ad, .header-auto-show').length) {
          ...
          const inside_header = total_scroll_height <= 150
          const passed_header = total_scroll_height >= 150
    ...
    // Stepper
    const stepperAdd = document.querySelectorAll('.stepper-add')
    const stepperSub = document.querySelectorAll('.stepper-sub')
    if (stepperAdd.length) {
      stepperAdd.forEach(el => el.addEventListener('click', (event) => {
        const currentValue = el.parentElement.querySelector('input').value
    ...
    // Link List Toggle
    const linkListToggle = document.querySelectorAll('[data-trigger-switch]:not([data-toggle-theme])')
    if (linkListToggle.length) {
      linkListToggle.forEach(el => el.addEventListener('click', (event) => {
        const switchData = el.getAttribute('data-trigger-switch')
        const getCheck = document.getElementById(switchData)
        getCheck.checked ? getCheck.checked = false : getCheck.checked = true
      }))
    }

    // Classic Toggle
    const classicToggle = document.querySelectorAll('.classic-toggle')
    ...
    if (toastTrigger.length) {
      toastTrigger.forEach(el => el.addEventListener('click', (event) => {
        const toastData = el.getAttribute('data-toast')
        let notificationToast = document.getElementById(toastData)
    ...
    let workingHours = document.querySelectorAll('.show-business-opened, .show-business-closed, .working-hours')
    if (workingHours.length) {
      ...
      const openClass = document.querySelectorAll('.show-business-opened')
      const closeClass = document.querySelectorAll('.show-business-closed')
      ...
      workingHours = document.querySelectorAll('.working-hours[data-day]')
      workingHours.forEach(function (entry) {
        const matchDay = entry.getAttribute('data-day')
        if (matchDay === day[0]) {
          const matchData = '[data-day="' + day[0] + '"]'
          if (now > day[1] && now < day[2] || now > day[3] && now < day[4]) {
    ...
    // Vibrate API
    const vibrateButton = document.querySelectorAll('[data-vibrate]')
    if (vibrateButton.length) {
      const startVibrating = document.getElementsByClassName('start-vibrating')[0]
      const stopVibrating = document.getElementsByClassName('stop-vibrating')[0]

      startVibrating.addEventListener('click', function () {
        const vibrateTime = document.getElementsByClassName('vibrate-demo')[0].value
      ...
      vibrateButton.forEach(el => el.addEventListener('click', (e) => {
        const vibrateTime = el.getAttribute('data-vibrate')
        window.navigator.vibrate(vibrateTime)
      }))
    }

    // Time Ads
    const timedAd = document.querySelectorAll('[data-timed-ad]')
    if (timedAd.length) {
      timedAd.forEach(el => el.addEventListener('click', (e) => {
        const timedAdTime = el.getAttribute('data-timed-ad')
        const timedAdData = el.getAttribute('data-menu')
        let timedAdTimer = timedAdTime
        const timerAdFunction = setInterval(function () {
          if (timedAdTimer <= 1) {
    ...
    // Auto Show Ads
    const autoAd = document.querySelectorAll('[data-auto-show-ad]')
    if (autoAd.length) {
      let autoAdTime = autoAd[0].getAttribute('data-auto-show-ad')
      const timerAdFunction = setInterval(function () {
        if (autoAdTime <= 1) {
          clearInterval(timerAdFunction)
          const autoAdId = autoAd[0].getAttribute('data-menu')
          document.getElementById(autoAdId).classList.add('menu-active')
          let autoAdCloseTime = autoAd[0].getAttribute('data-timed-ad')
    ...
    const readingTextDiv = document.querySelectorAll('.reading-progress-text')
    if (readingTextDiv.length) {
      const readingWords = readingTextDiv[0].innerHTML.split(' ').length
      const readingMinutes = Math.floor(readingWords / 250)
      const readingSeconds = readingWords % 60
    ...
    // Text Resizer
    const textSizeChanger = document.querySelectorAll('.text-size-changer')
    if (textSizeChanger.length) {
      const textSizeIncrease = document.querySelectorAll('.text-size-increase')
      const textSizeDecrease = document.querySelectorAll('.text-size-decrease')
      const textSizeDefault = document.querySelectorAll('.text-size-default')
    ...
    // QR Generator
    const qr_image = document.querySelectorAll('.qr-image')
    if (qr_image.length) {
      const qr_this = window.location.href
      const qr_auto = document.getElementsByClassName('generate-qr-auto')[0]
      const qr_api_address = 'https://api.qrserver.com/v1/create-qr-code/?size=200x200&data='
      if (qr_auto) { qr_auto.setAttribute('src', qr_api_address + qr_this) }
      const qr_btn = document.getElementsByClassName('generate-qr-button')[0]
      if (qr_btn) {
        qr_btn.addEventListener('click', function () {
          const get_qr_url = document.getElementsByClassName('qr-url')[0].value
          const qr_api_address = 'https://api.qrserver.com/v1/create-qr-code/?size=200x200&data='
          const qr_img = '<img class="mx-auto polaroid-effect shadow-l mt-4 delete-qr" width="200" src="' + qr_api_address + get_qr_url + '" alt="img"><p class="font-11 text-center mb-0">' + get_qr_url + '</p>'
    ...
    if (window.location.protocol === 'file:') {
      const linksLocal = document.querySelectorAll('a')
    ...
    // Search Page
    const searchField = document.querySelectorAll('[data-search]')
    if (searchField.length) {
      const searchResults = document.querySelectorAll('.search-results')
      const searchNoResults = document.querySelectorAll('.search-no-results')
      const searchTotal = document.querySelectorAll('.search-results div')[0].childElementCount
      const searchTrending = document.querySelectorAll('.search-trending')
      const clearSearch = document.querySelectorAll('.clear-search')[0]
      ...
      function searchFunction () {
        const searchStr = searchField[0].value
        const searchVal = searchStr.toLowerCase()
        if (searchVal !== '') {
          ...
          for (let i = 0; i < searchFilterItem.length; i++) {
            const searchData = searchFilterItem[i].getAttribute('data-filter-name')
            ...
            const disabledResults = document.querySelectorAll('.search-results div')[0].getElementsByClassName('disabled').length
      ...
      searchClick.forEach(el => el.addEventListener('click', (event) => {
        const trendingResult = el.querySelectorAll('span')[0].textContent.toLowerCase()
    ...
    if (navigator.canShare) {
      ...
      const shareMenu = document.querySelectorAll('[data-menu="menu-share"], [data-show-share]')
    ...
    if (contactForm.length) {
      ...
      form.onsubmit = function (e) {
        ...
        const mailField = document.getElementById('contactEmailField')
        const textField = document.getElementById('contactMessageTextarea')
    ...
    // Collapse Flip Icon
    const collapseBtn = document.querySelectorAll('[data-bs-toggle="collapse"]:not(.no-effect)')
    ...
    // Tabs
    const tabTrigger = document.querySelectorAll('.tab-controls a')
    if (tabTrigger.length) {
      tabTrigger.forEach(function (e) {
        if (e.hasAttribute('data-active')) {
          const highlightColor = e.parentNode.getAttribute('data-highlight')
          e.classList.add(highlightColor)
          e.classList.add('no-click')
        }
      })
      tabTrigger.forEach(el => el.addEventListener('click', (e) => {
        const highlightColor = el.parentNode.getAttribute('data-highlight')
        const tabParentGroup = el.parentNode.querySelectorAll('a')
    ...
    const autoActivate = document.querySelectorAll('[data-auto-activate]')
    ...
    // Check Age
    const checkAge = document.querySelectorAll('.check-age')
    if (checkAge.length) {
      checkAge[0].addEventListener('click', function () {
        const dateBirthday = document.querySelectorAll('#date-birth-day')[0].value
        const dateBirthMonth = document.querySelectorAll('#date-birth-month')[0].value
        const dateBirthYear = document.querySelectorAll('#date-birth-year')[0].value
        const age = 18
        ...
        const menuAge = document.querySelectorAll('#menu-age')
        const menuAgeFail = document.querySelectorAll('#menu-age-fail')
        const menuAgeOkay = document.querySelectorAll('#menu-age-okay')
    ...
    const addOfflineClasses = document.querySelectorAll('.offline-message')
    ...
    function onlinePage () {
      const anchorsEnabled = document.querySelectorAll('[data-link]')
      anchorsEnabled.forEach(function (e) {
        const hrefs = e.getAttribute('data-link')
        if (hrefs.match(/.html/)) { e.setAttribute('href', hrefs); e.removeAttribute('data-link', '') }
      })
    }

    // Defining Offline/Online Variables
    const offlineMessage = document.getElementsByClassName('offline-message')[0]
    const onlineMessage = document.getElementsByClassName('online-message')[0]
    ...
    const simulateOffline = document.querySelectorAll('.simulate-offline')
    const simulateOnline = document.querySelectorAll('.simulate-online')
    ...
    function updateOnlineStatus (event) { const condition = navigator.onLine ? 'online' : 'offline'; isOnline() }
    ...
    const isMobile = {
      ...
      any () { return (isMobile.Android() || isMobile.iOS()) }
    }
    ...
    if (isPWA === true) {
      ...
      if (!checkPWA.classList.contains('isPWA')) {
        ...
        const hours = pwaRemind * 24 // Reset when storage is more than 24hours
        const now = Date.now()
        const setupTime = localStorage.getItem(pwaName + '-PWA-Timeout-Value')
        if (setupTime === null) {
    ...
    let plugIdent, plugClass, plugMain, plugCall
    const plugLoc = 'plugins/'
    ...
    for (let i = 0; i < plugins.length; i++) {
      ...
      // Load Plugins
      const plugTrigger = document.querySelectorAll(plugins[i].trigger)
      if (plugTrigger.length) {
        const loadScript = document.getElementsByTagName('script')[1]
        const loadScriptJS = document.createElement('script')
        ...
        loadScriptJS.addEventListener('load', function () {
          ...
          if (plugins[i].call !== undefined) {
            const callFn = document.getElementsByTagName('script')[2]
            const callJS = document.createElement('script')
